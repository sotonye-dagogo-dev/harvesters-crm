...
export interface MeetingScheduleViewProps {
  /** The API query string appended to /api/meetings (e.g. "?groupId=xyz"). Leave empty or set to "" for all meetings. */
  meetingQueryParams?: string;
  /** The groupId to use when creating meetings. If not provided, the "Create Meeting" button posts without groupId. */
  createGroupId?: string;
  /** Route prefix for meeting detail links, e.g. "/leader" or "/superadmin" */
  routePrefix: string;
  /** Heading text */
  heading?: string;
  /** Sub-heading text */
  subHeading?: string;
  /** Whether the user can create meetings */
  canCreate?: boolean;
  /** Whether to show the biweekly generator (requires createGroupId) */
  canGenerateSchedule?: boolean;
  /** Optional group selector element rendered in the header area */
  groupSelector?: React.ReactNode;
}

// ─── Component ───────────────────────────────────────────────────────────────

export default function MeetingScheduleView({
  meetingQueryParams = "",
  createGroupId,
  routePrefix,
  ...
  const fetchMeetings = useCallback(async () => {
    try {
      const response = await fetch(`/api/meetings${meetingQueryParams}`);
      if (response.ok) {
  ...
  const handleCreateMeeting = async (values: MeetingFormValues) => {
    ...
    try {
      ...
      if (createGroupId) body.groupId = createGroupId;
  ...
  const handleGenerateBiweeklySchedule = async (
    ...
    try {
      ...
      const createPromises = generated.map((meeting) =>
        fetch("/api/meetings", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
      ...
      if (successCount === generated.length) {
        ...
        message.warning(
          `Created ${successCount} of ${generated.length} meetings`
        );
        fetchMeetings();
      }
    } catch {
  ...
  const getMeetingsForDate = (date: Dayjs) =>
    meetings?.filter((m) => dayjs(m.date).isSame(date, "day"));
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold text-ds-text-primary">{heading}</h2>
          <p className="text-ds-text-secondary">{subHeading}</p>
        </div>
        <div className="flex flex-wrap gap-2 w-full sm:w-auto">
          ...
          {canGenerateSchedule && createGroupId && (
            <AntButton
              ...
              >
                <div className="flex flex-col sm:flex-row items-start justify-between gap-3">
                  ...
                  <AntButton
                    ...
                    onClick={() =>
                      router.push(`${routePrefix}/meetings/${meeting.id}`)
                    }
                  >
                    View Details
                  </AntButton>
