...
remember we also wanted to create and integrate location based attendance functionality as well as member involvement in campaigns increases. We want the UX to still be seamless, the application needs to be easy to use and understand and void of complexity to the user. The API and DB ops need to maintain ACID principles
- users (members as well as other types) should be able to document their campaign contributions, alongside the other metrics like conversion recorded for them. What this means is, for a campaign (ongoing or previous) the user should be able to , enter the number of views they got and upload screenshot evidence of their WhatsApp status. Possible future update could involve the optional inclusion of other social media platforms
- Location sensitive and logically sound attendance tracking system for meetings and events. The campus locations would be saved (enter mock details for now). Users would have access to a page to mark their attendance for meetings or events. Location is requested for, If physically within 100m of the location, then attendance is marked as "Physically Present" and "Marked by self", if not within 100m, mark as "Virtuallly Present" "Mrked by self", fallback option is being marked by someone of higher level in hierarchy having that user in jurisdiction, having "Physically present" and "MArked by {actor}". The attendance feature should also detect device and restrict members from using device to mark attendance successfully more than once for a particular event or meeting.

 We also want more intelligent, modular and dynamic coding basically all across the web app, also making modifications easier, more seamless and scalability and refactoring easier.